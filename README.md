...
넣는 칸은 *제 설정 저장소*를 이어 붙이는 자리다. 설치는 그것을 **`~/repos/<저장소 이름>`**
으로 받는다. 거기서 **갈래가 둘이고 둘 다 정당하다** — 가르는 자는 **파일 이름 하나**, 받아온
저장소 뿌리의 **`bootstrap-vdi.sh`** 다.

| 그 이름의 파일이 | 설치가 하는 일 |
|---|---|
| **없다** | **받아 두고 끝낸다** — `받아 뒀다 — C:\Users\…\repos\<이름>`. 저장소만 들고 다니려면 이것이 전부다 |
| **있다** | 그것을 Git Bash 로 부르고 **그 뒤는 그 저장소가 든다** — git 신원 · 형제 저장소 · 키 · 로그인 · 배포 |

⚠ **없는 갈래도 조용히 넘어가지 않는다** — 부트스트랩이 왜 안 돌았는지를 그 자리에서 말한다.
