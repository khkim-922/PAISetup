...
5. **기다리면 설치가 창을 대신 띄운다** — 사내면 VS Code, 사외면 Claude 데스크탑

무엇을 띄울지는 고르는 것이 아니라 **설치가 잰 자리**가 정한다. 사내는 게이트웨이라 그 주소로
말이 통하는 것이 확장·CLI 뿐이고, 사외는 구독 로그인이라 데스크탑 앱이 앞문이다.

⚠ **VS Code 가 이미 떠 있으면 안 띄운다.** 창은 뜰 때 환경을 한 번 복사하고 그 뒤에 바뀐 것은
안 따라오는데, 떠 있는 VS Code 에 창을 하나 더 내면 그 창은 **낡은 프로세스 안에서** 난다 —
방금 심은 키를 못 보면서 화면에는 열어 준 것으로 보인다. 그 자리에서는 설치가 그렇게 말하고
손을 뗀다: **전부 닫고 새로 연다.** 도는 창을 설치가 죽이지는 않는다 — 저장 안 한 것이 있다.
