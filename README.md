...
├─ install.cmd               백신이 위 exe 를 물었을 때 물러설 자리. 같은 설치 창을 띄운다
...
1. zip 을 받아 **폴더를 통째로 풀어** 아무 데나 둔다 (압축 안에서 바로 누르면 안 된다 —
   설치가 옆 파일들을 못 찾는다)
2. **`Setup.exe` 를 더블클릭한다** — 설치 창이 뜬다.
   처음 받은 파일이라 윈도우가 *「Windows에서 PC를 보호했습니다」* 를 띄우면
   **[추가 정보] → [실행]** 을 누른다. 백신이 아예 지워 버렸으면 `install.cmd` 를 누른다 —
   같은 설치 창이 뜨고, 검은 창이 뒤에 하나 같이 뜨는 것만 다르다
