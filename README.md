...
`.claude/` 는 반대다: **꺼진 것이 기본**이고 켜야 깔린다.
...
| `agents/` | `~/.claude/agents/` — **이 폴더는 안 실려 있다.** 만들어 넣으면 같이 깔린다 |

자리가 비어 있으면 **건너뛰고 실패로 세지 않는다.** 그래서 넷 중 일부만 바꿔 넣어도 된다 —
스킬만 제 것을 쓰고 룰은 안 쓰는 식으로. `agents/` 가 지금 그 꼴이다: 폴더가 없어 설치가
「에이전트 — 이 폴더에 없다」 한 줄을 찍고 넘어간다. **결함이 아니라 빈 자리다** — 만든 사람의
에이전트는 안 싣기로 했고, 그 자리에 제 것을 넣으면 그대로 깔린다.
