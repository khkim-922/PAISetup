...
풀면 이렇게 생겼다. **누를 것은 `Setup.exe` 하나**고 나머지는 그것이 쓰거나, 깔린 뒤에 읽는
것이다.

```text
├─ Setup.exe              ← 누르는 것. 이것만 더블클릭한다
├─ install.cmd               위 파일이 백신에 지워졌을 때 물러설 자리. 똑같이 동작한다
...
1. zip 을 받아 **폴더를 통째로 풀어** 아무 데나 둔다 (압축 안에서 바로 누르면 안 된다 —
   설치가 옆 파일들을 못 찾는다)
2. **`Setup.exe` 를 더블클릭한다** — 설치 창이 뜬다. 검은 창은 안 뜬다.
   **그 파일이 안 보이면 백신이 지운 것이다** — 옆의 `install.cmd` 를 누르면 된다.
   똑같이 돌고 검은 창도 안 뜬다. 다른 것은 아이콘뿐이다
...
따라가지는 않는다. 그때는 새 zip 을 받아 같은 자리에 풀고 `Setup.exe` 를 다시 누른다.
