...
넣는 칸은 *제 설정 저장소*를 이어 붙이는 자리다. 규약은 **파일 이름 하나**다 — 그 저장소
뿌리에 **`bootstrap-vdi.sh`** 가 있어야 한다. 설치는 저장소를 받은 뒤 그 파일을 찾아
Git Bash 로 부르고, **그 뒤는 그 저장소가 든다**(git 신원 · 형제 저장소 · 키 · 배포).

⚠ 그 이름의 파일이 없으면 설치가 `! 저장소에 bootstrap-vdi.sh 가 없다` 로 **실패를 찍는다.**
받아 오기만 하고 끝내지 않는다 — 넘길 자리가 없는 것을 성공으로 읽지 않기 때문이다.
**그런 저장소면 칸을 비워 둔다.**
...
빈 파일로 그 이름만 맞추는 길도 있지만, 그러면 **넘겨받고 아무 일도 안 하는 자리**가 되고
이미 값을 치른 함정 셋을 처음부터 다시 겪는다 — 창이 PATH 를 못 물려받는 것, 윈도우가 깔아 둔
파이썬 껍데기가 「있음」을 내는 것, 마지막 `echo` 로 끝내면 판정이 늘 `0` 이 되는 것.
씨앗의 곁말에 그 셋이 **왜**와 함께 박혀 있다.
