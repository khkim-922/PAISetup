...
#   든다. **설치본 자체를 새 릴리스로 갈지는 아직 않는다** — 지금 그 일을 드는 것은 설치
#   창의 자가 갱신뿐이다(맨 끝 칸 ⚠).
...
# ⚠ **설치본 자체를 새 릴리스로 갈지는 아직 않는다 — 그러니 그렇게 적지도 않는다.** 여기가
#   드는 것은 **이 판이 아는 제품들을 최신으로 두는 일**까지고, 받은 exe 를 릴리스에 오른
#   해시와 대조해 갈아 끼우는 일은 설치 창의 자가 갱신이 든다(「새 판이 있습니다」 물음).
# ⚠ **무인으로 못 할 일이라서가 아니다** — 해시 대조가 그 자리를 들 수 있다. 안 지었을
#   뿐이고 지을지는 안 갈렸다(2026-09-18). 못 한다고 읽히게 적으면 그 물음이 닫힌다.
