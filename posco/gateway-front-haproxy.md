...
> 트래픽에 무엇을 하는지**를 든다.
>
> ⚠ **전 기능·전 설정 항목은 여기 안 적는다** — 원본은 상류의 공식 설정 매뉴얼이다
...
> 원본이고, **그 어느 장도 이 층은 안 든다.**

## 어떻게 알아냈나 — 오류 본문이 지문이다

우리가 받은 504 의 본문이 HAProxy 기본 오류 파일과 **바이트까지 같다.**
...
앞단이 스스로 만들어 내는 응답들이다. **본문을 보면 누가 냈는지 갈린다.**
...
| `timeout check` · `tarpit` · `client-fin` · `server-fin` · `client-hs` | 그 밖 | 안 걸린다 |

## ⚠ 우리를 끊은 시한 — `timeout server`
...
**「무응답 시간」이지 「첫 바이트까지」가 아니다.** 조각이 오면 다시 0 부터 재고, 스트림이 끝날 때까지
계속 잰다. 그래서 이 제품의 표준 처방이 **심장박동**이다 — 뒤쪽이 할 말이 없을 때도
주기적으로 무언가를 흘려 그 시계를 계속 되돌린다.
...
| 압축·헤더 재작성·ACL | 요청 본문이나 헤더를 바꿀 수 있다. 안 겪었지만 못 겪는다는 뜻은 아니다 |

## 되보내기가 켜지면 — 우리에게 무엇이 걸리나
...
그런데 **실질적으로는 대부분 막혀 있다.** 막는 것이 셋이고 성질이 다 다르다.

| 무엇이 막나 | 어떻게 |
|---|---|
| **버퍼** | 되보내려면 「copy the whole request into it」이고 「**Requests not fitting in a single buffer will never be retried**」다. `tune.bufsize` 기본이 16KB 인데 우리 요청 본문은 실측 90~200KB — **큰 호출(고치기·렌더)은 애초에 대상이 아니다** |
| **구조** | 답은 우리에게 **하나만** 오고 도구는 게이트웨이가 아니라 앱이 실행한다 — 문서가 두 번 고쳐지지 않는다. 잃는 것은 토큰과 시간뿐이다 |
| **앱의 시한** | `response-timeout` 되보내기는 **앞단의 시한이 먼저 만료돼야** 일어난다. 앱이 먼저 끊으면 만료될 일이 없다 — 도구 바퀴가 제 시간 예산으로 끊는 자리가 그것이다 |

⚠ **셋째는 지금 비긴 상태다** — 앱 예산도 앞단 무응답 시한도 300초라, 첫 바퀴에서는 어느
...
⚠ **Claude 쪽에는 이 손잡이가 없다** — 생각 조각이 게이트웨이를 안 넘어온다([`ENV-posco.md`](ENV-posco.md)).
로컬 프록시가 흘리는 keepalive 주석(결정 0044)은 클라이언트 쪽으로만 가므로, 게이트웨이 뒤쪽을 재는 이 시한과는 무관하다.

## ⚠ 180초 컷은 이 층이 아니다

스트림이 **신호 없이** 끊기는 **180초** 시한이 따로 있다. **여기가 아니다** — 이 층 실측은
`timeout server` **300초**라 값이 안 맞는다(300 ≠ 180).

**그 시한은 P-GPT 앱의 것이다(실측 2026-09-17).** 끊긴 스트림의 마지막 바이트에 사유가 적혀 있다:
`HTTP request execution did not complete before the specified timeout configuration: 180000 millis`.
앱이 **밖으로 나가는 호출**에 건 요청 실행 시한이고 **스트림 길에서만** 걸린다 — 같은 앱의
비스트리밍은 이 층의 300초까지 산다. ⚠ **왜 경로마다 다른지는 그쪽 안이라 밖에서 못 본다.**

**그 시한의 자세한 기록은 [`ENV-posco.md`](ENV-posco.md)** 가 든다 — P-GPT 층에서 잰 것이라 거기 있다.
여기가 드는 것은 **이 층이 아니라는 사실**뿐이다: 다음 사람이 180초를 앞단에 붙이지 않게.

⚠ **처방도 다르다** — 180초는 침묵 시한이 아니라 요청 실행 시한이라, 위 *「선을 조용하게 두지 않는다」* 로는
안 넘는다. 넘는 길은 스트림이 아닌 길로 보내는 것이다([`ENV-posco.md`](ENV-posco.md) · 결정 0051).
...
- **층이 더 있을 수 있다.** 확인된 것은 「앞단 HAProxy」와 「P-GPT」 둘이고, 그 사이나
  위에 다른 층이 있는지는 모른다
