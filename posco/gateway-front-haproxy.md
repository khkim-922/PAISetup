...
**그 자의 원본은 [`ENV-posco.md`](ENV-posco.md)** 다 — P-GPT 층에서 잰 것이라 거기 산다.
...
- **겹이 더 있을 수 있다.** 확인된 것은 「앞단 HAProxy」와 「P-GPT」 둘이고, 그 사이나
  위에 다른 층이 있는지는 모른다
- **180초 자는 P-GPT 앱이다 — 갈렸다(실측 2026-09-17).** 절단의 마지막 바이트가 사유를 실토한다:
  `HTTP request execution did not complete before the specified timeout configuration: 180000 millis`.
  앱이 **밖으로 나가는 호출**에 건 요청 실행 시한이고 **스트림 길에서만** 물린다 — 같은 앱의
  비스트리밍은 이 층의 300초까지 산다. 앞단이 아니라는 기존 가름(300 ≠ 180)은 그대로 버틴다.
  ⚠ **왜 경로마다 다른지는 그쪽 안이라 밖에서 못 본다** ([`ENV-posco.md`](ENV-posco.md))
