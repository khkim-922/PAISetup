...
| **(우리 것)** `/v1/messages` 에 `"stream": true` 가 오면 상류엔 **`"stream": false`** 로 보내고, 기다리는 동안 위 주석을 흘리다, 답이 오면 SSE 를 지어 낸다(`message_start` → 블록마다 `content_block_*` → `message_delta` → `message_stop` · 도구 호출은 `input_json_delta` 하나에 `input` 통째). 몸은 그 한 칸 말고 안 바뀐다 — `cache_control` 도 키 차례도 그대로다. `PGPT_PROXY_UNSTREAM=0` 이면 옛 길 | ⚠ **지금은 끄고 쓴다 — 긴 도구 인자를 깨뜨린다** (결정 0051). 회사 실측 2026-09-17: 벽은 넘었다(직결 `stream:true` 는 180.06초에 게이트웨이가 `180000 millis` 를 실토하며 끊고, 이 길은 300.01초까지 살아 앞단 `504` 를 SSE `error` 로 옮겼다 · `unstreamed` 18회 · 첫 주석 15.01초). **그런데 같은 판에서 긴 도구 인자가 통째로 유실됐다** — 긴 `Edit`·`Write`·`Agent` 호출이 `provided as unknown`(값이 안 옴)으로 지고, 손잡이를 끄면 같은 호출이 한 번에 통한다. 짧은 인자는 양쪽 다 산다. 자리는 `input_json_delta` 통째 담기가 의심되나 안 갈렸다 |
