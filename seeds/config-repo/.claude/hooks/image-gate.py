...
부르는 자리 — 홈 `~/.claude/settings.json` 의 PreToolUse. 심는 손은 `session-start.sh --install`
(홈 SessionStart 훅과 같은 자리 · 같은 규율). 셸 껍데기가 stdin 에 그림 낌새(확장자·screenshot·zoom)가
있을 때만 파이썬을 띄운다 — 매 Read 마다 프로세스를 띄우지 않기 위해서다.
