...
#
# 원본은 claude-config/.claude/hooks/session-start-body.sh 다. **저장소마다 사본을 두지 않는다**
# (claude-config 0061) — 저장소마다 서는 것은 문지기 `session-start.sh` 이고, 문지기가 붙은
# claude-config 의 이 몸통(없으면 홈 씨앗의 몸통)을 찾아 저장소를 넘기며 부른다.
# 그래서 이 파일을 고쳐도 형제 저장소 커밋이 따라 나오지 않는다.
#
...
# ⚠ **문지기가 부른 자리는 저장소를 넘겨받는다**(`SESSION_START_REPO` · 0061). 몸통은 claude-config
#   (또는 홈 씨앗)에 살아 제 경로가 대상 저장소가 아니다 — 문지기가 제 경로로 잡은 저장소를 넘긴다.
#   받은 글자를 그대로 쓴다: 꼴 굳히기(`home_hook_root`)는 아래 한 자리가 들고, 여기서 `pwd` 로
#   먼저 눌러 버리면 그 자리를 재는 검사(`check-home-hook.sh` ③)가 딴 꼴을 못 밟는다.
# ⚠ **받은 자리에서 끊는다(unset).** 이 몸통은 deploy.ps1 을 부르고 그것이 claude-config 훅을 다시
#   부른다 — 값이 환경에 남으면 그쪽이 형제 저장소를 제 저장소로 잡는다. 이어 받을 자리(아래 설치
#   기록의 다시 띄우기)는 값을 명시해 넘긴다.
_self="${BASH_SOURCE[0]:-$0}"
PROJECT_DIR=""
if [ -n "${SESSION_START_REPO:-}" ] && [ -d "$SESSION_START_REPO" ]; then
  PROJECT_DIR="$SESSION_START_REPO"
fi
unset SESSION_START_REPO
...
if [ -n "$ASKED" ] && [ -z "${SESSION_LOG:-}" ] && [ -f "$_self" ] && mkdir -p "$HOME/.claude/logs" 2>/dev/null; then
  ...
  SESSION_START_REPO="$PROJECT_DIR" bash "$_self" "$@" 2>&1 | tee -a "$SESSION_LOG"
...
CONFIG_ROOT=""; ADR_GEN=""; GATES_HOME=""
...
# ── 이 실행이 PC 전체 일을 맡나 (claude-config 0062) ─────────────────────────────
#    PC 전체 일 — 홈에 규범 · 룰 · 에이전트 · 씨앗 밀기 · 개인 설정(git 전역 설정 · 키) · 홈 훅과
#    신뢰 목록 심기 · 전역 도구 선언 대조. 저장소가 몇이든 PC 에 한 번이면 되는 일이다.
#    맡는 자리는 넷이다 — 로그인 자동 실행(`deploy.ps1` → `--install`) · 설정 저장소를 연 세션 ·
#    저장소 밖에서 연 세션(홈 훅이 설정 저장소 훅을 부른다, 0043) · 리모트 설정 스크립트(`--install`).
...
$(find "$CONFIG_ROOT/$_sd" -type f 2>/dev/null)
EOF
    done
    # 견주기 판은 여기까지다 — 아래 판 줄은 「민 뒤에 적는다」가 뜻이라 안 민 판에는 적을 것이 없다.
    [ "$HOME_SYNC" = push ] || return 0
...
pc_wide_notice() {
  ...
  echo "$PROJECT_NAME: ⚠ 이 PC 홈에 깔린 규범 · 룰 파일이 claude-config 에 있는 최신과 다르다($_hl — 없거나 옛것) — 이 세션에는 최신 규칙이 안 실려 있다. claude-config 를 한 번 열면 다음 세션부터 맞춰진다"
}
...
# ── ⚠ **무는 목록은 굳혀서 정렬하고 겹침을 걷는다** (#78) ──────────────────────────
#   이 자리가 무는 것은 「이 기계에 붙은 전역 선언 **집합**」이고, 집합에는 순서가 없다. 그런데
#   `global_conf_list` 는 **부르는 저장소의 것을 맨 앞에** 놓는다 — 그쪽의 뜻이고(겹침을 절
#   이름으로 걷는 순서 · 제 선언이 이긴다) 건드리지 않는다. **두 명제가 같은 목록을 쓰던 것이
#   결함이었다**: 앞자리가 저장소마다 달라 해시가 갈리고, 게다가 제 저장소 선언이 앞자리와
#   글롭에 **두 번** 든다(글롭은 제 것도 훑는다 — 저쪽은 절 이름으로 걷어 무해하지만 이쪽은
#   파일을 그대로 다 물어 두 번 씹는다).
#   실측 2026-09-22 (집 PC · 저장소 여섯) — 고치기 전 저장소별 지문이 **넷으로 갈렸고**, 도장은
#   기계 하나라 그중 하나와만 맞았다. 그래서 다른 저장소를 열면 반드시 전역 걸음이 서고, 그
#   걸음이 **100초**라 확장의 60초 벽(0043)에서 죽었다 — 죽은 판이 제 값으로 도장을 갱신하니
#   다시 열면 가벼워, 증상이 「첫 실행만 실패」로 보였다. 정렬·중복 제거만으로 여섯이 한 값이
#   된다(실측 `769b07fb…`).
#   ⚠ **경로 꼴을 먼저 굳힌다 — 굳히지 않으면 겹침이 안 걷힌다.** 윈도우에서 한 파일이 두 꼴로
#     사는데(`/c/Users/…` 는 `PROJECT_DIR` 파생 · `C:/Users/…` 는 글롭 파생 — `home_hook_root` 가
#     `cygpath -m` 을 타서다) 문자열이 다르니 `sort -u` 가 둘을 남긴다. 드라이브 문자까지 한 꼴로
#     눌러야 같은 파일이 한 줄이 된다.
#   ⚠ **`cygpath` 를 파일마다 부르지 않는다 — 순정 치환이다.** 실측 같은 PC: `cygpath` 열한 번이
#     **2.8초**, 셸 치환은 **0.3초**다. 위 「프로세스는 sha256sum 하나」 규율이 여기서도 선다.
#   ⚠ **정렬은 이 자리 안에서 끝낸다.** `global_conf_list` 에 정렬을 넣으면 선언을 읽는 순서가
#     바뀌어 「제 저장소의 선언이 이긴다」가 깨진다 — 같은 이름의 절을 형제가 들 때 누가 이길지가
#     알파벳순이 된다. 고칠 것은 **무는 방식**이지 목록이 아니다.
# ── 한 글자를 대문자로 — **프로세스 없이** (#78) ──────────────────────────────────
#   드라이브 문자를 굳히는 자리에서만 쓴다. `tr` 을 띄우면 파일마다 프로세스가 하나 더 붙고
#   (윈도우 실측: 열한 번에 2.8초 — `cygpath` 와 같은 값), `${v^}` 는 bash 4 전용이라
#   `sh` 로 불린 자리에서 죽는다. `case` 는 내장이라 둘을 다 피한다.
_upper_one() {
  case "$1" in
    a) printf A ;; b) printf B ;; c) printf C ;; d) printf D ;; e) printf E ;;
    f) printf F ;; g) printf G ;; h) printf H ;; i) printf I ;; j) printf J ;;
    k) printf K ;; l) printf L ;; m) printf M ;; n) printf N ;; o) printf O ;;
    p) printf P ;; q) printf Q ;; r) printf R ;; s) printf S ;; t) printf T ;;
    u) printf U ;; v) printf V ;; w) printf W ;; x) printf X ;; y) printf Y ;;
    z) printf Z ;; *) printf '%s' "$1" ;;
  esac
}
...
global_fingerprint() {
  global_conf_list | {
    while IFS= read -r _gf; do
      # 꼴 굳히기 — `/c/…` → `C:/…`. 드라이브 문자는 대문자로 눌러 글롭이 내는 꼴과 맞춘다.
      case "$_gf" in
        /?/*) _gd="${_gf#/}"; _gd="${_gd%%/*}"
              case "$_gd" in
                [a-z]) _gd="$(_upper_one "$_gd")" ;;
              esac
              _gf="$_gd:/${_gf#/?/}" ;;
      esac
      printf '%s\n' "$_gf"
...
#    한 세션에 이 훅이 두 번 불릴 수 있다 — 저장소 설정이 거는 것과 홈 훅이 거는 것. 둘 다 제
#    저장소를 당기면 같은 당김이 두 번이다(실측 2026-09-09 회사 PC: 한 훅 출력에 같은 경고가 아홉 줄 ·
#    그때는 형제마다 설정 저장소까지 당겨 더 겹쳤다 — 0062 가 그 당김을 걷었다).
...
#   ② 홈 규범·훅 배선을 새로 민다 — 멱등·싼 것이라 매 세션. **PC 전체 일이라 설정 저장소 세션과
...
if [ -d "$PROJECT_DIR/.githooks" ]; then
  ...
  if hooks_wired; then
    ...
    if [ ! -f "$_gc" ]; then
      ...
      gate "커밋 게이트" off "러너를 못 찾았다 — 이 저장소 · claude-config · 홈 씨앗 어디에도 gates-run.sh 가 없다. 커밋이 **아무것도 안 재고** 지나간다"
    else
      ...
      if [ -n "$_gone" ]; then
        gate "커밋 게이트" off "선언에 있는데 조각을 못 찾았다:$_gone — 이 PC 의 claude-config 가 뒤처졌나 본다(선언의 오타일 수도 있다). 그 검사는 못 잰다 · 커밋은 안 막는다"
      elif [ -z "$_on" ]; then
