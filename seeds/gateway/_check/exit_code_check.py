...
⚠ **빌려 온 자다** — 진본은 claude-config `seeds/check/_check/exit_code_check.py`(같은 저장소 · 같은 판)이고,
...
AST 만 읽는다 — 그 갈래는 곁 `shell_exit_check.py` 가 문다) · `sys.exit` 에 안 물린
...
def main(argv):
    ...
    edge("**`.mjs`·`.sh`·`.ps1` 은 안 든다** — 파이썬 AST 만 읽는다. "
         "그 갈래의 나가는 자리는 곁 `shell_exit_check.py` 가 든다.")
