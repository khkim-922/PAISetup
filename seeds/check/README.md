...
줄끝·BOM·곁말 블록을 뺀 글자가 같은지 문다. **곁말 자리는 머리말 문단의 끝, 첫 빈 줄 바로 앞이다**
— 파이썬은 독스트링 첫 문단 끝에, `.mjs` 는 머리 주석 블록 안이 아니라 **그 블록 뒤 첫 빈 줄 앞에
`//` 줄로**(블록 주석 안에 두면 곁말 블록이 주석을 통째로 삼키고, 파일 머리에 두면 닫는 빈 줄이 몸통에
더해져 진본과 갈린다 — 실측 아뜰리에 A1). 진본이 움직이면 다시 받고 곁말의 커밋을 올린다.
